class API::V0::CsvReportsController < API::V0::BaseController
  ...
  def create
    ...
    if address.blank?
      raise API::V0::Error.new(I18n.t("views.csv_reports.flashes.missing_house"), 422)
    end

    # Error out if there are no reports extracted.
    rows = CsvReport.extract_rows_from_spreadsheet(spreadsheet)
    if rows.blank?
      raise API::V0::Error.new(I18n.t("views.csv_reports.flashes.missing_visits"), 422)
    end
    ...
    rows.each_with_index do |row, row_index|
      ...
      unless type == "n"
        ...
        if eliminated_at.present? && eliminated_at.future?
          raise API::V0::Error.new(I18n.t("views.csv_reports.flashes.elimination_date_in_future"), 422)
        end

        if eliminated_at.present? && eliminated_at < parsed_current_visited_at
          raise API::V0::Error.new(I18n.t("views.csv_reports.flashes.elimination_date_before_inspection_date"), 422)
        end
