...
class Visit < ActiveRecord::Base
  ...
  def self.segment_locations_by_date_and_type(location_ids, start_time, end_time, scale)
    ...
    visits       = Visit.select("id, visited_at, location_id").where("csv_id IS NOT NULL OR source = ?", "mobile").where(:location_id => location_ids).order("visited_at ASC")
    ...
    inspections = Inspection.order("position ASC").where("csv_id IS NOT NULL OR source = ?", "mobile").where(:visit_id => visits.pluck(:id)).select(:visit_id, :identification_type)
